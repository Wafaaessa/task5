...
export default function CreateChatRoomForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    // Store room name and password in Firestore
    await roomRef.set({
      password: password
    });
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>CreateChatRoom Page</title>
      ...
      <div className="container pt-5 ">
        <div className="row justify-content-center mt-5 pt-5">
          <div className="col-md-6 mt-5 p-4 border  rounded-3 contain-create"  >
            <h2 className="text-center title">Create Chat Room</h2>
            {error && <p className="error text-center text-danger">{error}</p>}
            <form onSubmit={handleSubmit} className="form-group">
            <div className="row mb-3">
        <div className="col-md-3 my-3">
          <label htmlFor="roomName" className="fw-bolder">Room Name:</label>
        </div>
      ...
      <div className="row">
        <div className="col-md-3 offset-md-5">
          <button type="submit" className="btn btn-info w-100">Create Room</button>
        </div>
      </div>
            </form>
          </div>
